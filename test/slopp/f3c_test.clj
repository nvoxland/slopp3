...
(deftest project-wide-test-run                          ; F-3c1
  (let [sess (api/open!)]
    (try
      (seed! sess)
...
(deftest query-eval-surfaces-errors                     ; F-3c2
  (let [sess (api/open!)]
    (try
      (seed! sess)
...
(deftest references-cross-namespaces                    ; F-3c3
  (let [sess (api/open!)]
    (try
      (seed! sess)
...
(deftest traceless-group-verification-covers-all-touched-nses ; F-3c5
  (let [sess (api/open!)]
    (try
      (seed! sess)
