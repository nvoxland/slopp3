...
(deftest query-deps-transitive-callee-tree
  (let [sess (api/open!)]
    (try
...
(deftest fix-declares-moves-and-deletes
  (let [sess (api/open!)]
    (try
...
(deftest ns-rename-rewrites-the-world
  (let [sess (api/open!)]
    (try
      (api/ingest! sess 'sg.core core-src)
      (api/ingest! sess 'sg.util util-src)
...
(deftest extract-forms-to-a-new-namespace
  (let [sess (api/open!)]
    (try
      (api/ingest! sess 'sg.core core-src)
