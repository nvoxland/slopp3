...
(deftest ^:external subform-edit-touches-only-what-it-names
  (let [sess (external/open!)]
    (try
      (ops/ingest! sess 'sf.core target)
      (testing "replace one inner expression; siblings and comments untouched"
        (let [r (ops/edit-subform! sess 'sf.core 'price
      ...
      (testing "ambiguity, absence, and degenerate results are clean errors"
        (ops/add-form! sess 'sf.core "(defn dup [a] (+ (inc a) (inc a)))")
        (is (re-find #"2 times" (:error (ops/edit-subform! sess 'sf.core 'dup
                                                           "(inc a)" "(dec a)"))))
        (is (:error (ops/edit-subform! sess 'sf.core 'price "(nope)" "(yep)")))
        (is (re-find #"dialect"
                     (:error (ops/edit-subform! sess 'sf.core 'dup
                                                "(+ (inc a) (inc a))"
                                                "(eval a)")))))
...
(deftest ^:external fragment-matches-suggest-the-enclosing-form
  ...
  (let [sess (external/open!)]
    (try
      (ops/ingest! sess 'fg.core
                   (str "(ns fg.core)\n\n"
                        "(defn g \"G.\" [x]\n"
                        "  (let [a (inc x)\n"
                        "        b (* 2 a)]\n"
                        "    (+ a b)))\n"))
      (let [r (ops/edit-subform! sess 'fg.core 'g
                                 "a (inc x)\n        b (* 2 a)]" "ignored")]
        (is (:error r))
