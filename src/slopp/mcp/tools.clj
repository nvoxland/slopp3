...
{:name "js_dep"
   ...
   {:name "store_doctor"
    ...
    :inputSchema {:type "object" :properties {}}}
   {:name "ui_serve"
    :description "Serve THIS project's own API listener — /api/* as JSON, plus the shape of that API as EDN at /api/contracts. It has NO pages in it: `/` answers 404, so a human handed this url sees JSON. The screens live in the UI HUB, a separate application that renders every page and fronts this project at /p/<slug>/ (D-ui-hub part 4) — when the answer is for a human, hand over session_brief's :ui-hub, not its :ui. Served on the LIVE session, so warranty and observed examples are the ones this session actually has; a process that opened the same store fresh would show every form as covered by nothing. Returns {:url :port}. `port` overrides the ui.port capability, which is unset by default and DERIVED from the store dir so several projects on one machine never collide — 7359 is ui.hub-port, a different setting; `stop: true` shuts it down. Serving again EVICTS the running server rather than hunting for a free port, and a port someone else holds comes back as a sentence, not a stack trace."
    :inputSchema {:type "object"
...
{:name "compile_client"
   ...
   {:name "generate_client"
    :description "Generate the typed CLIENT (D-web-contracts part 2): one edit-PROTECTED :cljs namespace of typed fetch wrappers, validating the request OUT and the response IN against a shared :cljc schema. Two sources. WITHOUT `from`: read the endpoints THIS store serves; the target ns defaults to the client/generated-ns config, else app.client.api (`ns` overrides). WITH `from` (a URL serving a published contract, e.g. http://host/api/contracts): generate against an API this store CONSUMES — writes a :cljc contracts namespace of the published schemas alongside the client, reads no other store, and refuses a contract version it does not know. An EXPLICIT step (like compile_client, not on every edit); marks the ns :cljs so compile_client picks it up, and with client/auto-compile on schedules a background recompile. Endpoints whose schema can't ship to the client (non-:cljc, or a missing var) are SKIPPED and reported in :problems. Regenerate, never hand-edit."
    :inputSchema {:type "object"
                  :properties {:ns {:type "string"}
                               :from {:type "string"}}}}
