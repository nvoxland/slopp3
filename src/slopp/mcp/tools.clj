...
{:name "js_dep"
   ...
   {:name "store_doctor"
    ...
    :inputSchema {:type "object" :properties {}}}
   {:name "ui_serve"
    :description "Serve THIS project's own API listener — /api/* as JSON, plus the shape of that API as EDN at /api/contracts. It has NO pages in it: `/` answers 404, so a human handed this url sees JSON. The screens live in the UI HUB, a separate application that renders every page and fronts this project at /p/<slug>/ (D-hub part 4) — when the answer is for a human, hand over session_brief's :hub, not its :ui. Served on the LIVE session, so warranty and observed examples are the ones this session actually has; a process that opened the same store fresh would show every form as covered by nothing. Returns {:url :port}. `port` overrides the slopp.api.port capability, which is unset by default and DERIVED from the store dir so several projects on one machine never collide — 7359 is slopp.hub.port, a different setting; `stop: true` shuts it down. Serving again EVICTS the running server rather than hunting for a free port, and a port someone else holds comes back as a sentence, not a stack trace."
    :inputSchema {:type "object"
