...
| Rename a QUALIFIED KEYWORD (`:a/x` -> `:b/x`) | `rename_sweep` moves the literals correctly and currently gets `:keys` DESTRUCTURING wrong in BOTH directions, so sweep, then `query_search` for `/keys` and read every hit before trusting it. It does NOT follow a qualified `{:a/keys [x]}` — the qualifier lives in the MAP KEY, not the symbol, so the destructure is left naming the old key while the schema and every call site moved, and the function silently receives nil. And it DOES rewrite an UNQUALIFIED `{:keys [x]}` that has nothing to do with your rename — it hoists the symbol out into `{:b/keys [x]}`, changing which key that form reads, which is how a state map holding `:x` quietly starts being read as `:b/x`. Both failures present as nil arriving silently rather than as an error, so the only tests that can catch them are the ones exercising the value END-TO-END — which for a session, a projection or a subprocess means `^:external`, and those are exactly the ones a write DEFERS. Do not read the write's green as coverage here |
