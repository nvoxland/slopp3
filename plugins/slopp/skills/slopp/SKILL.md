...
| Reorder / delete / undo | `edit_move` / `edit_delete_form` / `edit_revert`. **`query_depends {on "ns/name"}` BEFORE every delete** — one call, and it is the whole question. A delete whose form still has a caller is ACCEPTED, not refused: the write commits, then the image reload fails and the store boots NOWHERE — every later call included, since the loader is all-or-nothing. The answer looks like a refusal (`error: restart load failed … Unable to resolve symbol: X`) and is not one, so re-reading it as "that didn't go through" is how the delete gets made twice. Recovery is `undo {deltas 1}` — but `undo` walks back only YOUR OWN writes, so a delete made under a different agent (a `--call` script, another session) answers `no writes of yours to undo` while looking straight at it; that case needs `episode_revert`. Never judge a form's role by its NAME; the check is cheaper than the recovery |
